...
var setupPaymentMethod = function(setupIntentSecret, paymentMethod, element) {
  ...
  var couponId = document.querySelector('#coupon').value;
...
function createSubscription(customerId, paymentMethodId, couponId) {
...
function pauseSubscription(subscriptionId) {
  return fetch('/pause-subscription', {
    ...
    body: JSON.stringify({
      subscriptionId: document.getElementById('pause').getAttribute('value')
    })
  })
    .then(function(response) {
      return response.json();
...
function cancelSubscription(subscriptionId) {
  return fetch('/cancel-subscription', {
    ...
    body: JSON.stringify({
      subscriptionId: document.getElementById('cancel').getAttribute('value')
    })
  })
    .then(function(response) {
      return response.json();
...
var orderComplete = function(subscription) {
  ...
  document.querySelector('#pause').value = subscriptionJson;

  if (subscription.pause_collection) {
    var pauseHeader = document.getElementById('pause-header');
    pauseHeader.classList.remove('hidden');
    document.querySelector('.pause-status').textContent =
      subscription.pause_collection.behavior;
  }
  var pauseButton = document.getElementById('pause');
  pauseButton.setAttribute('value', subscription.id);

  var cancelButton = document.getElementById('cancel');
  cancelButton.setAttribute('value', subscription.id);
};
